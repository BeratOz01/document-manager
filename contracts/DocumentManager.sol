...
contract DocumentManager is Ownable {
    /**
     *  @dev Mapping of document hash to document owner
    ...
    /**
     *  @dev Mapping of address to total amount of documents owned by that address
    ...
    /**
     * @dev Nested mappings for see document details
    ...
    /// @notice Events
    event Approve(bytes32 hash, address approvedAddress);
    ...
    function ownerOf(bytes32 hash) public view returns (address) {
        ...
        require(_owners[hash] != address(0), "Hash does not exist");
        return _owners[hash];
    }
